...
public CzechStemmerAgressive(){} // constructor
	...
	private void removeDerivational(StringBuffer buffer) {
		...
		if(len > 7){
			if(buffer.substring( len-5 ,len).equals("ion\u00e1\u0159")){ // -ionár 
		...
		if(len > 5){
			...
			if(buffer.substring( len-3 ,len).equals("kyn")|| 
				    ...
				    buffer.substring( len-3 ,len).equals("k\u00e1\u0159")|| //kár
				    buffer.substring( len-3 ,len).equals("n\u00e9\u0159")|| //nér
		 ...
		 if(len > 4){
			...
			if(buffer.substring( len-2 ,len).equals("ec")|| 
				    ...
				    buffer.substring( len-2 ,len).equals("\u00e9\u0159")){  //-ér
						
				    buffer.delete( len-1 , len);
				    palatalise(buffer);
				    return;
			}
			if(buffer.substring( len-2 ,len).equals("\u00ed\u0159")|| //-ír
	...
	private void removeDiminutive(StringBuffer buffer) {
		...
		if( len> 6){
			 if(buffer.substring( len-4,len).equals("e\u010dek")||      //-ecek
			    buffer.substring( len-4,len).equals("\u00e9\u010dek")||    //-écek
			    buffer.substring( len-4,len).equals("i\u010dek")||         //-icek
			    buffer.substring( len-4,len).equals("\u00ed\u010dek")||    //ícek
			    ...
			    return;
		            ...
		            if( buffer.substring( len-4,len).equals("\u00e1\u010dek")|| //ácek
		...
		if( len> 5){
			    if(buffer.substring( len-3,len).equals("e\u010dk")||   //-eck
			       buffer.substring( len-3,len).equals("\u00e9\u010dk")||  //-éck 
			       buffer.substring( len-3,len).equals("i\u010dk")||   //-ick
			       buffer.substring( len-3,len).equals("\u00ed\u010dk")||    //-íck
			    ...
			    if(buffer.substring( len-3,len).equals("\u00e1\u010dk")||  //-áck
	...
	private void removeComparative(StringBuffer buffer) {
		...
		if( (len> 5)&&
			(buffer.substring( len-3,len).equals("ej\u0161")||  //-ejš
			 buffer.substring( len-3,len).equals("\u011bj\u0161"))){   //-ejš
	...
	private void palatalise(StringBuffer buffer){
		...
		if( buffer.substring( len- 3 ,len).equals("\u010dt\u011b")||     //-cte
		     ...
		     buffer.substring( len- 3 ,len).equals("\u010dt\u00ed")){   //-ctí
						
		     buffer.replace(len- 3 ,len, "ck");
		     return;
		}
		if( buffer.substring( len- 2 ,len).equals("\u0161t\u011b")||   //-šte
